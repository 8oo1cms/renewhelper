...
<body>
  ...
  <div class="mecha-panel p-1 !border-l-0">
    <el-table :key="tableKey" :data="pagedList" style="width:100%" v-loading="loading" :row-class-name="tableRowClassName" size="large" @sort-change="handleSortChange" @filter-change="handleFilterChange" :default-sort="{prop: 'daysLeft', order: 'ascending'}">       
        ...
        <el-table-column :label="t('nextDue')" min-width="260" prop="daysLeft" sortable="custom" column-key="daysLeft" :filters="nextDueFilters">
            <template #default="scope">
                <div v-if="scope.row.enabled">
                    <div class="flex flex-col gap-1.5">
                        <div class="flex items-center gap-2">
                            <div class="font-mono text-base font-bold leading-none tracking-tight" :class="getDaysClass(scope.row.daysLeft)">
                                {{ formatDaysLeft(scope.row.daysLeft) }}
                            </div>
                            <div v-if="scope.row.useLunar" class="text-[9px] font-bold text-purple-600 bg-purple-50 border border-purple-200 px-1 py-[2px] leading-none whitespace-nowrap">
                                {{ t('lunarCal') }}
                            </div>
                        </div>
                        <div class="grid grid-cols-3 gap-1 text-[10px] font-mono text-textDim">
                            ...
                            <div>
                                ...
                                <div class="truncate">{{ scope.row.nextDueDate }}</div>
                            </div>
                            <div class="text-right">
                                <div class="uppercase text-[9px] text-gray-400">{{ lang === 'zh' ? '距今' : 'LEFT' }}</div>
                                <div class="truncate" :class="getDaysClass(scope.row.daysLeft)">
                                    {{ scope.row.daysLeft }}
                                    {{ lang === 'zh' ? '天' : 'd' }}
                                </div>
                            </div>
                        </div>
                        <div v-if="scope.row.useLunar && scope.row.nextDueDateLunar" class="text-[9px] text-blue-500/80 font-mono leading-tight">
    ...
    <script>
        ...
        const frontendCalc = {
            l2s(l) {
                ...
                const base = new Date(Date.UTC(1900, 0, 31));
                const target = new Date(base.getTime() + days * 86400000);
                return { year: target.getUTCFullYear(), month: target.getUTCMonth() + 1, day: target.getUTCDate() - 1 };
            },
            addPeriod(l, val, unit) {
                let { year, month, day, isLeap } = l;
                if (unit === 'year') {
